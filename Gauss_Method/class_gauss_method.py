...
#RI, RJ,     RK = cal_R(Re, f, height, lat, lst)
			...
			solar_system_ephemeris.set('jpl')
			#t = Time(['2020-09-10', '2020-09-15', '2020-09-20'], scale='utc')
			r1, r2, r3 = get_body_barycentric('earth', time)
			print(r1)
...
#14 calculate the true anomaly, the Eccentric anomaly, the Mean anomaly with time
		...
		if self.obj == 'planet':
			pluto_T = 100000 # day
			t = np.linspace(0, pluto_T*3, 1000)
		else:
...
# calculation xyz and XYZ
		...
		for t1, u, w, z in zip(t, x, y, z):
			...
			Xxdata.append([t1, u, w, z])
			xXdata.append([t1, tranxX[0], tranxX[1], tranxX[2]])

		self.TXYZ = xXdata
		self.txyz = Xxdata

		return self.txyz, self.TXYZ
	...
	def plotly_plot(self, inp_data):
		...
		df = pd.DataFrame(inp_data, columns=['time', 'x', 'y', 'z'])
		'''
...
for i, name in enumerate(sat_Nlist):
	...
	breakpoint()
	sat_inp_data[name] = TXYZ
Name_list = []
T_list = []
X_list = []
Y_list = []
Z_list = []
for target in sat_inp_data.keys():
	t, x, y, z = sat_inp_data[target]
	print(t.shape)
	num = t.shape[0]
	name = np.array([name for i in range(num)])
	print(name.shape)
	breakpoint()
...
sat_inp_data = np.concatenate([NAme.reshape((len(T),1)), T.reshape((len(T),1)), X.reshape((len(T),1)), Y.reshape((len(T),1)), Z.reshape((len(T),1))], axis=1)
print(sat_inp_data)
breakpoint()
fig, fig2 = sat.plotly_plot(sat_inp_data)
...
def add_rpraaT(ele):
	...
	print(f'This is the cal_keplerian_equation order : [i, h, e, Omega, omega, theta]')
	return trans_ele
...
for key in planet_Nlist:
	xyz, XYZ = planet.cal_kepler_equation(planet_data[key])
	inp_data[key] = XYZ
planet.plot_3d(inp_data)
'''



"""
R 구하는 것만 달리하면 하면됨. 나머지 함수는 모두 동일함.
"""

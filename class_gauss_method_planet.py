...
from usefultools import deg2rad, rad2deg, sec2hour

class orbit():
	def __init__(self):
		'''
		self.time = [0, 118.10, 237.58] # sec
		self.ra = [43.537, 54.420, 64.318]  # deg
		self.ra = deg2rad(self.ra)
		self.dec = [-8.7833, -12.074, -15.105]  # deg
		self.dec = deg2rad(self.dec)
		self.lst = [44.506, 45.0, 45.499]  # deg
		self.lst = deg2rad(self.lst)
		self.lat = 40.0  # deg
...
# recalling the para
	...
	def cal_r2v2_vect(self, observation_data, latitude, height=0):
		...
		self.lat = latitude # deg : all same
...
#13 obtain the orbital elements with r2 and v2
	def cal_orbit_ele(self):
		r = np.linalg.norm(self.r2_vect) # km
		v = np.linalg.norm(self.v2_vect) # km/s
		vradial = np.dot(self.r2_vect, self.v2_vect) / r  # km/s
		h_vect = np.cross(self.r2_vect, self.v2_vect) # km2/s
		...
		e_vect = 1 / self.mu * ((v**2 - self.mu/r)*self.r2_vect - r*vradial*self.v2_vect) # unitless
		...
		if vradial >= 0 :
			self.theta = np.arccos(np.dot(e_vect, self.r2_vect) / (self.e*r)) # rad
			theta1 = np.arccos(1/self.e*(self.h**2/(self.mu*r) -1))
		else:
			self.theta = 2*np.pi - np.arccos(np.dot(e_vect, self.r2_vect) / (self.e * r))
		...
		self.elements = (self.i, self.h, self.e, self.Omega, self.omega, self.theta)
		print('i, h, e, Omega, omega, theta')
		print(self.elements)
		return self.elements

	def cal_kepler_equation(self):
...
# calculation xyz and XYZ
		...
		for u, w, z in zip(x, y, z):
			...
			#print(type(tran))
			Xxdata.append([u, w, z])
			xXdata.append(list(tranxX))

		self.XYZ = np.array(xXdata).T
		self.xyz = np.array(Xxdata).T

		return self.xyz, self.XYZ

	def plot_3d(self, data=False):
		if data is False:
			data = self.XYZ
		...
		N = len(data[0, :])
		ani = animation.FuncAnimation(fig, update, N, fargs=(self.XYZ, position), interval=5000 / N, blit=False)
		# ani.save('matplot003.gif', writer='imagemagick')
		plt.show()
		print('end')
		'''
		ax.plot(self.XYZ[0, :], self.XYZ[1, :], self.XYZ[2, :])
		position, = ax.plot(self.XYZ[0, 0:1], self.XYZ[1, 0:1], self.XYZ[2, 0:1], 'ro')
...
class satel_orbit(orbit):
	def __init__(self, observation_data, latitude, height=0):
		super().__init__(observation_data, latitude, height)
		self.mu = 3  # km3/sec2
		self.Re = 6  # km ; the equatorial radius of the earth
		self.f = 0.  # unitless; the flattening factor
...
lat = deg2rad(lat)
height = 1.  # km
observation_data = [time, ra, dec, lst]

sat = orbit() # all degree, km
sat.cal_r2v2_vect(observation_data, lat, height)
sat.cal_orbit_ele()
sat.cal_kepler_equation()
sat.plot_3d()
print(sat.elements)
